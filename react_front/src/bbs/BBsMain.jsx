...
// 서버와 통신을 할떄 사용할 여러 설정값을 지정
// 데이터를 서버로부터 요청할떄 사용할 URL값
...
class BBsMain extends Component {
  ...
  /*
    ...
    state로 선언한 변수는 this.setState()라는 method를 사용해서
    변수 값을 update 수행해야 하고 

    state 변수는 자식 component에 전달하여 rendering 용도로 사용할수 있다.
  ...
  */
  ...
  render() {
    /*
        ...
        코드가 약간 번잡스러워 지기 떄문에
        부모 컨테이너에서 비구조화 작업을 수행한 후 변수를 전달해주도록 한다.
...
※ props 형태의 변수는 절대 변경할 수 없는 형태가 되고 단순히 rendering 하는
용도로만 사용해야 한다.

변화가 있는 데이터를 자식이 rendering 하도록 하기 위해서는 
부모 컨테이너에서 state 형식의 변수로 만들고 어떤 형태를 통하여
데이터를 변경하면 그 데이터가 자식에게 전파되어 화면에 변화된 모습을
render 한다.
