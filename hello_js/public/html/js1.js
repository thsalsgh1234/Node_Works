...
// console.log(값,값,값) : 각각의 값들을 형변환 하지말고 있는 그대로 출력하라
console.log("객체", std);
...
var sum = 0;
// forEach문은 배열을 한개씩 순회하면서 요소들을 callback 함수에 전달하며 코드를 수행한다.
// forEach를 이용하여 요소를 연산한 다음
// forEach가 끝난 후 값을 조회하면 forEach의 순회에서 계산된 결과가 정확히
// 조회된다는 보장이 없다 forEach는 비동기 방식이기 떄문에
arrNumber.forEach(function(item, index, originalArray) {
  sum += item; // 1번 연산코드
  sum += originalArray[index]; //1번과 같은 연산코드
});
// 배열 순회후에 연산 결과를 보장받으려면 전통적 for를 사용해야 한다.
...
// 배열을 순회하면서 각 요소를 callback함수에 전달하고 callback 함수가 return 하는 값들을
// 모아서 다른 배열로 생성해 준다.
...
// find는 배열요소중 값 찾기
// 화살표 함수에서 다른 코드가 없이 return 문 한줄만 있을경우
// 함수 몸체의 {} 를 생략하고 return 키워드를 사용하지 않는다.
// 화살표 함수의 매개변수가 1개만 있을때에는 () 없이 사용한다.
//  매개변수가 1개도 없을떄는 빈(blank) 괄호만 반드시 사용해야 한다.
//  매개변수가 2개이상 일떄는 필수적으로 괄호로 묶는다.
...
const arrNumber3 = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 12, 4, 1, 2, 3, 4, 1, 2, 3];
...
// forEach 수행이 끝난 후 연산결과를 조회했을때 연산결과의 정확도를 보장할수 없는 문제를
// 해결한 함수이다.
const reduce = arrNumber4.reduce((acc, item) => {
  return acc + item;
});
console.log("reduce : ", arrNumber4, "합계: ", reduce);

// 1차원 배열일 경우 배열을 정렬하는 기능
const sortString = arrString.sort();
console.log("정렬", sortString);

// callback 함수를 사용해서 역순정렬
// callback 함수에서 결과 조건을 연산 한 후
// -1이나 1을 return하면 정렬을 asc, desc로 변환할수 있다.
