...
class BBsMain extends Component {
  ...
  componentDidMount() {
    ...
    // 이전에 callback 함수가 종료되고 timer시간이 경과하면 다시 반복하여 실행하라
    this.timer = setInterval(() => this.fetchBBsList(), 5000);
  }
  // 혹시 화면에 그려지는 여러 컴포넌트들의 연결이 꺠지면 실행되는 method인데
  ...
  fetchBBsList = () => {
    ...
    // ES6 js에 새로운 Ajax method가 있는데 새로운 method를 사용해서 데이터를 조회해오기
    fetch(BBS_FETCH_URL)
      .then(response => {
        // (문자열형으로 리턴된) 가져온 데이터를
  ...
  render() {
    /*
        state에 선언된 변수를 사용하려면 {this.state.bbsList}와 같이사용을 해야하는데
        이것을 분해를 해두면 {bbsList}만 써도된다.

        필드영역에 선언된 this.state를 분해하여 그중에 bbsList를 별도로 추출을 해두면
        tag 코드에서 사용할때 {bbsList} 형식으로 사용할 수 있다.
    */
